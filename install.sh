...
author=milisp
# github=https://github.com/milisp/xray
...
# root
[[ $EUID != 0 ]] && err "Current user is not ${yellow}ROOT.${none}"
...
# systemd
[[ ! $(type -P systemctl) ]] && {
    err "This system is missing ${yellow}(systemctl)${none}, try to fix this by running: ${yellow} ${cmd} update -y; ${cmd} install systemd -y ${none}"
}
...
show_help() {
    ...
    echo -e "  -l, --local-install             Fetch installation script locally, use current directory"
    echo -e "  -p, --proxy <addr>              Use proxy to download, e.g., -p http://127.0.0.1:2333 or -p socks5://127.0.0.1:2333"
...
check_status() {
    ...
    [[ ! -f $is_pkg_ok ]] && {
        ...
        msg err "Try to manually install dependencies: $cmd update -y; $cmd install -y $pkg"
        is_fail=1
    }
...
pass_args() {
    while [[ $# -gt 0 ]]; do
        ...
        -f | --core-file)
            [[ -z $2 ]] && {
                err "($1) Missing required parameter, correct usage example: [$1 /root/$is_core-linux-64.zip]"
            } || [[ ! -f $2 ]] && {
        ...
        -p | --proxy)
            [[ -z $2 ]] && {
                err "($1) Missing required parameter, correct usage example: [$1 http://127.0.0.1:2333 or -p socks5://127.0.0.1:2333]"
            }
            proxy=$2
            shift 2
            ;;
        -v | --core-version)
            [[ -z $2 ]] && {
                err "($1) Missing required parameter, correct usage example: [$1 v1.8.1]"
            }
            is_core_ver=v${2#v}
            shift 2
            ;;
    ...
    [[ $is_core_ver && $is_core_file ]] && {
        err "Cannot specify both ${is_core_name} version and ${is_core_name} file."
    }
}
...
exit_and_del_tmpdir() {
    ...
    [[ ! $1 ]] && {
        msg err "Oops.."
        msg err "An error occurred during the installation process..."
...
main() {
    ...
    [[ -f $is_sh_bin && -d $is_core_dir/bin && -d $is_sh_dir && -d $is_conf_dir ]] && {
        err "Detected that the script is already installed, if you need to reinstall use the command ${green} ${is_core} reinstall ${none}."
    }
    ...
    [[ $is_core_file ]] && {
        ...
        msg warn "${yellow}${is_core_name} file used > $is_core_file${none}"
    }
    # local dir install sh script
    [[ $local_install ]] && {
        >$is_sh_ok
        msg warn "${yellow}Fetching installation script locally > $PWD ${none}"
    }

    timedatectl set-ntp true &>/dev/null
    [[ $? != 0 ]] && {
        msg warn "${yellow}\e[4mReminder!!! Unable to set automatic time synchronization, it may affect the use of VMess protocol.${none}"
    }
    ...
    if [[ $is_core_file ]]; then
        ...
        [[ $? != 0 ]] && {
            msg err "${is_core_name} file failed verification."
            exit_and_del_tmpdir
        }
